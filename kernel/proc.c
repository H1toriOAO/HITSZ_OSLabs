...
freeproc(struct proc* p)
{
    ...
    if (p->k_pagetable) {
        proc_free_kpagetable(p->k_pagetable, p->kstack, p->sz);
    }
...
void proc_free_kpagetable(pagetable_t pagetable, uint64 kstack, uint64 sz)
{
...
void userinit(void)
{
    ...
    p->sz = PGSIZE;
    copypage(p->pagetable, p->k_pagetable, 0, p->sz);
...
int growproc(int n)
{
    ...
    if (n > 0) {
        ...
        if (copypage(p->pagetable, p->k_pagetable, p->sz, sz) < 0) {
            return -1;
        }
    } else if (n < 0) {
...
int fork(void)
{
    ...
    if (copypage(np->pagetable, np->k_pagetable, 0, p->sz) < 0) {
        freeproc(np);
        release(&np->lock);
        return -1;
    }
